...
public class Town extends JPanel implements KeyListener {
	private imageIcon backGround = (new ImageIcon("src/townBackground.png")).getImage();
	private Game game;
	...
	public void paintComponent(Graphics g) {
		...
		g.drawImage(p.getImageIcon(), 0, 0, 50, 50, null); // Draws player
	}
	...
	public void keyTyped(KeyEvent e) {
		...
		else if (e.getKeyChar() == 's' && pLocation.getY() < backGround.getHeight()) {
			pLocation.setLocation(pLocation.getX(), pLocation.getY()+1);
		}
		else if (e.getKeyChar() == 'd' && pLocation.getX() < backGround.getWidth()) {
			pLocation.setLocation(pLocation.getX()+1, pLocation.getY());
		}
		else if (e.getKeyChar() == 'a' && pLocation.getX() > 0) {
			pLocation.setLocation(pLocation.getX()-1, pLocation.getY());
		}
		if (pLocation.getY() == backGround.getHeight()) {
